...
Due to a computational power limit, the training data set will contains only 1000 observations. 
...
number of effective sample (ess). Without going into the details of those two metrics, we 
can see that as long as rhat approaches 1 and the number of effective sample is close to 
the sample size of 2000, the sampling algorithm works well and we have credible estimates 
for the coefficients in the model
...
each system by the mean value of each ball types. We can make it more complicated by building a hidden 
markov chain to inversely infer the missing data by cross learning between the hittype and recorded 
angles. However, we will keep it simple for now

After that, we are free to focus entirely on missing data of speed variable. The imputation for 
the variable is explained in the next part
